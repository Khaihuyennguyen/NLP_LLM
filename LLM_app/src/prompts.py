# Promts for the chatbot and evaluation
...
def load_chat_prompt(f_name: Union[pathlib.Path, str] = None) -> ChatPromptTemplate:
    # A union can be used as Union[A, B] 
    # to indicate that the object can have type A or B.
    if isinstance(f_name, str) and f_name:
        f_name = pathlib.Path(f_name)
    if f_name and f_name.is_file():
        template = json.loads(f_name.open("r"))
    else:
        logger.warning(f'No chat prompt provided. Using default chat prompt from {__name__}')
        template = {
